...
model DeSchipjesSwitch
  extends IOCSmod.Optimization.Interface;
  Blocks.Demand.Clusters.DeSchipjes        buildings(
                                                    addDummyEquation=
        addDummyEquation)
    annotation (Placement(transformation(extent={{40,0},{60,20}})));

  Blocks.EnergyHub.SwitchingFramework
                                 enerHub(
...
equation
  ...
  connect(buildings.port_a, enerHub.port_b) annotation (Line(points={{56,0},{56,
          -40},{-36,-40},{-36,0}}, color={0,127,255}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)));
end DeSchipjesSwitch;
