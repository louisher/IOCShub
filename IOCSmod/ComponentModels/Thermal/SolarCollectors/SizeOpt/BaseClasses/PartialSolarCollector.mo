...
partial model PartialSolarCollector "Partial model for solar collectors"
 extends Buildings.Fluid.Interfaces.LumpedVolumeDeclarations;
  ...
  parameter input Modelica.Units.SI.Area totalArea "Total area of panels in the simulation" annotation (Dialog(group=
          "Area declarations", enable=(nColType == Buildings.Fluid.SolarCollectors.Types.NumberSelection.Area)));

  parameter Modelica.Units.SI.Area totalArea_nominal "Nominal area of STC, used for calculation of dp_nominal and m_flow_nominal" annotation (Dialog(group=
          "Area declarations", enable=(nColType == Buildings.Fluid.SolarCollectors.Types.NumberSelection.Area)));
  ...
  Buildings.Fluid.SolarCollectors.Types.NumberSelection.Area
  ...
  parameter input Integer nPanelsPar=0 "Number of array panels in parallel"
  ...
  IOCSmod.ComponentModels.Thermal.SizeOpt.Tank.MixingVolumeOpt
                                             vol[nSeg](
    each nPorts=2,
...
protected
  ...
  parameter input Modelica.Units.SI.Area ATot_internal=nPanels_internal*per.A
  ...
  parameter input Real nPanels_internal=totalArea/per.A "Number of panels used in the simulation";
  parameter input Real nPanelsSer_internal=
  ...
  parameter input Real nPanelsPar_internal=
  ...
  final parameter Modelica.Units.SI.MassFlowRate m_flow_nominal_final(
      displayUnit="kg/s") = nPanelsPar_internal_nominal*per.mperA_flow_nominal*per.A
  ...
  final parameter Modelica.Units.SI.PressureDifference dp_nominal_final(
      displayUnit="Pa") = nPanelsSer_internal_nominal*per.dp_nominal
    "Nominal pressure loss across the system of collectors";


  parameter Real nPanels_internal_nominal=totalArea_nominal/per.A "Nominal number of panels used in the simulation";
  parameter Real nPanelsSer_internal_nominal=
    if sysConfig == Buildings.Fluid.SolarCollectors.Types.SystemConfiguration.Series then
      nPanels_internal_nominal
    else if sysConfig == Buildings.Fluid.SolarCollectors.Types.SystemConfiguration.Parallel then
      1
    else
      nPanels_internal_nominal/nPanelsPar_internal_nominal "Nominal number of panels in series";
  parameter Real nPanelsPar_internal_nominal=
    if sysConfig == Buildings.Fluid.SolarCollectors.Types.SystemConfiguration.Parallel then
      nPanels_internal_nominal
    else if sysConfig == Buildings.Fluid.SolarCollectors.Types.SystemConfiguration.Series then
      1
    else
      nPanelsPar "Nominal number of panels in parallel";

  Modelica.Units.SI.MassFlowRate m_flow_nominal_actual
...
equation
  m_flow_nominal_actual= nPanelsPar_internal*per.mperA_flow_nominal*per.A;
...
<p>This component is a partial model of a solar thermal collector. It can be expanded to create solar collector models based on either ASHRAE93 or EN12975 ratings data. </p>
<p>Update: this model is used to optimize the total area of the collectors. For this, nominal mass_flow rates and pressure losses are calculated based on a nominal size. This means pressure drops are not correctly calculated. </p>
<p><b>References</b></p>
<p><a href=\"https://energyplus.net/assets/nrel_custom/pdfs/pdfs_v23.2.0/EngineeringReference.pdf\">EnergyPlus 23.2.0 Engineering Reference</a> </p>
</html>", revisions="<html>
<ul>
<li>
January 29, 2025, by Louis Hermans:<br/>
Changed the parameter nPanelsSer_internal and nPanelsPar_internal to use nPanels_internal instead of nPanels otherwise issues (e.g. wrong nominal mass flow) appear when the amount of panels by the user is set with the total area.<br/>
