...
model ConservationEquationOpt
  ...
  Medium.BaseProperties medium(
    ...
    X(start=X_start),
    d(start=rho_start),
    preferredMediumStates=true) "Medium properties";
  ...
  parameter input Modelica.Units.SI.Volume fluidVolume "Volume";
  final parameter input Modelica.Units.SI.HeatCapacity CSen=
...
equation
  ...
  if computeCSen then
    ...
    else // custom implementation that does not cause an algebraic loop for dry air
      medium.u  = U/(m*mSenFac);
    end if;
  else
    U = m*medium.u;
  end if;
...
<ul>
<li>
April, 2025, by Louis Hermans:<br/>
Removed start attributes of <code>U, m, mX, mX</code>. In instance <code>medium</code> set <code>preferredMediumStates=true</code> to ensure temperature is used as a state in TACO. 
This allows to change the input parameter volume during optimization.
</li>
<li>
