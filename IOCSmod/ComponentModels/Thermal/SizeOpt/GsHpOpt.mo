...
model GsHpOpt "Model of a GSHP with optimizable borefield model and passive cooling heat exchanger in series with condensor:
  ...
  extends IOCSmod.ComponentModels.BaseClasses.ElecThermInterface;
    ...
    parameter Real inv_cost_bor(fixed=false, start=0) "Investment cost per unit of Size (€/kW)" annotation(Dialog(group="Investment cost"));
    parameter Real interest_rate(fixed=false, start=0) annotation(Dialog(group="Investment cost"));
    parameter Integer lifetime_hp(fixed=false, start=0) "Lifetime in years" annotation(Dialog(group="Investment cost"));
    parameter Integer lifetime_bor(fixed=false, start=0) "Lifetime in years" annotation(Dialog(group="Investment cost"));
    parameter Integer observation_time(fixed=false, start=0) "Observation time in years" annotation(Dialog(group="Investment cost"));

    parameter Boolean hasReg=true "Boolean to create external ports for regeneration" annotation(Dialog(group="Configuration"));
    ...
    parameter
    .IOCSmod.ComponentModels.Thermal.Borefields.BaseClasses.HeatTransfer.Data.GroundResponseDataTemplate
    groTemResDat(kappa(kappa(each fixed=false)), rCel(rCel(each fixed=false)))
    "Ground response parameters" annotation (Dialog(group="Borefield"));
  ...
  IDEAS.Fluid.Sensors.TemperatureTwoPort senTHexCooConOut(
    ...
    allowFlowReversal=false) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={20,12})));
    Borefields.TwoUTubes_ContOpt                               borefield(
    tLoaAgg=3600,
    nSeg=1,
    borFieDat=borFieDat,
    allowFlowReversal=false,
    ...
    z0=0,
    dT_dz=0,
    groTemResDat=groTemResDat) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={0,90})));
  ...
  IOCSmod.ComponentModels.BaseClasses.Investment inv_hp(
    inv_cost=inv_cost_hp,
    interest_rate=interest_rate,
    lifetime=lifetime_hp,
    observation_time=observation_time)
    annotation (Placement(transformation(extent={{80,110},{100,130}})));

  BaseClasses.Investment inv_bor(
    inv_cost=inv_cost_bor*borefield.borFieDat.conDat.nBor,
    interest_rate=interest_rate,
...
equation
  ...
  connect(Expr_mFlowCon.y, pumGsHpCon.m_flow_in) annotation (Line(points={{-49,-26},
          {-54,-26},{-54,-20},{-60,-20},{-60,-26}}, color={0,0,127}));
