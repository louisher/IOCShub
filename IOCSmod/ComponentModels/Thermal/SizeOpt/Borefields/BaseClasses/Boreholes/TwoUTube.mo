...
model TwoUTube "Double U-tube borehole heat exchanger"
  extends
    IOCSmod.ComponentModels.Thermal.SizeOpt.Borefields.BaseClasses.Boreholes.BaseClasses.PartialBorehole;

  IOCSmod.ComponentModels.Thermal.SizeOpt.Borefields.BaseClasses.Boreholes.BaseClasses.InternalHEXTwoUTube
    intHex[nSeg](
    redeclare each final package Medium = Medium,
    each final borFieDat=borFieDat,
    each hSeg=depth/nSeg,
    final dp1_nominal={if i == 1 and borFieDat.conDat.borCon == IDEAS.Fluid.Geothermal.Borefields.Types.BoreholeConfiguration.DoubleUTubeParallel
        ...
        IDEAS.Fluid.Geothermal.Borefields.Types.BoreholeConfiguration.DoubleUTubeSeries
...
<p>Model of a double U-tube borehole heat exchanger. The borehole heat exchanger is vertically discretized into <i>n<sub>seg</i></sub> elements of height <i>h=h<sub>Bor</sub>&frasl;n<sub>seg</i></sub>. Each segment contains a model for the heat transfer in the borehole, with a uniform borehole wall boundary temperature given by the <span style=\"font-family: Courier New;\">port_wall</span> port. </p>
<p>The heat transfer in the borehole is computed using a convective heat transfer coefficient that depends on the fluid velocity, a heat resistance between each pair of pipes, and a heat resistance between the pipes and the borehole wall. The heat capacity of the fluid and the heat capacity of the grout are taken into account. The vertical heat flow is assumed to be zero. </p>
<p><br><b>To allow parameter optimization:</b></p>
<p>- Change extension to <span style=\"font-family: Courier New; color: #ff0000;\">InternalHEXTwoUTube</span> model with input parameters.</p>
<p>- Set the hSeg parameter in the BorHol to depth/nSeg</p>
</html>", revisions="<html>
<ul>
<li>July 24, 2025, by Louis Hermans:<br>Change extened model to partialBorehole with input parameters. Set hSeg of borHol to depth/nSeg.</li>
<li>
