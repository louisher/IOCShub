...
model OneUTube "Single U-tube borehole heat exchanger"
  ...
  IOCSmod.ComponentModels.Thermal.Borefields.BaseClasses.Boreholes.BaseClasses.InternalHEXOneUTube
    intHex[nSeg](
...
<p>Model of a single U-tube borehole heat exchanger. The borehole heat exchanger is vertically discretized into <i>n<sub>seg</i></sub> elements of height <i>h=h<sub>Bor</sub>&frasl;n<sub>seg</i></sub>. Each segment contains a model for the heat transfer in the borehole, with a uniform borehole wall boundary temperature given by the <span style=\"font-family: Courier New;\">port_wall</span> port. </p>
<p>The heat transfer in the borehole is computed using a convective heat transfer coefficient that depends on the fluid velocity, a heat resistance between the two pipes, and a heat resistance between the pipes and the borehole wall. The heat capacity of the fluid and the heat capacity of the grout are taken into account. The vertical heat flow is assumed to be zero. </p>
<p><br><b>To allow parameter optimization:</b></p>
<p>- Change extension to InternalHEXOneUTube model with input parameters.</p>
