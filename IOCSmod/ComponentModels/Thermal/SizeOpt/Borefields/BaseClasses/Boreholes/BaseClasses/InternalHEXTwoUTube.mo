...
model InternalHEXTwoUTube
  ...
  extends
    IOCSmod.ComponentModels.Thermal.SizeOpt.Borefields.BaseClasses.Boreholes.BaseClasses.EightPortHeatMassExchanger(
  ...
  Modelica.Blocks.Sources.RealExpression RVol3(y=(RConv3Expr.y +
        resistanceCalculator.RCondGro_val)/hSeg)
    "Convective and thermal resistance at fluid 1"
    annotation (Placement(transformation(extent={{-12,-60},{-26,-76}})));

  Modelica.Blocks.Sources.RealExpression RVol4(y=(RConv4Expr.y +
        resistanceCalculator.RCondGro_val)/hSeg)
               "Convective and thermal resistance at fluid 1"
    annotation (Placement(transformation(extent={{-68,12},{-54,28}})));
  IOCSmod.ComponentModels.Thermal.SizeOpt.Borefields.BaseClasses.Boreholes.BaseClasses.InternalResistancesTwoUTube
    intRes2UTub(
    hSeg=hSeg,
    borFieDat=borFieDat,
    ...
    T_start=TGro_start,
    Rgg1_val=resistanceCalculator.Rgg1_val,
    Rgg2_val=resistanceCalculator.Rgg2_val)
  ...
  ResistanceCalculator_TwoUTube resistanceCalculator(
    hSeg_dummy=hSeg_dummy,
    borFieDat=borFieDat,
    cpMed=cpMed,
    kMed=kMed,
    muMed=muMed)
    annotation (Placement(transformation(extent={{-60,92},{-40,112}})));
  Modelica.Blocks.Sources.RealExpression RConv4Expr(y=
        ...
        cpMed=cpMed,
        m_flow=m1_flow,
        m_flow_nominal=m4_flow_nominal)*hSeg_dummy)
    "Convective and thermal resistance at fluid 1"
    annotation (Placement(transformation(extent={{-68,26},{-54,42}})));
  Modelica.Blocks.Sources.RealExpression RConv1Expr(y=
    ...
    "Convective and thermal resistance at fluid 1"
    annotation (Placement(transformation(extent={{-30,66},{-16,82}})));
  Modelica.Blocks.Sources.RealExpression RConv2Expr(y=
    ...
    "Convective and thermal resistance at fluid 1"
    annotation (Placement(transformation(extent={{72,-16},{86,0}})));
  Modelica.Blocks.Sources.RealExpression RConv3Expr(y=
        IOCSmod.ComponentModels.Thermal.Borefields.BaseClasses.Boreholes.BaseClasses.Functions.convectionResistanceCircularPipe(
        hSeg=hSeg_dummy,
        rTub=borFieDat.conDat.rTub,
        eTub=borFieDat.conDat.eTub,
        ...
        cpMed=cpMed,
        m_flow=m1_flow,
        m_flow_nominal=m3_flow_nominal)*hSeg_dummy)
    "Convective and thermal resistance at fluid 1"
    annotation (Placement(transformation(extent={{-26,-96},{-12,-80}})));
equation
...
<p>Model for the heat transfer between the fluid and within the borehole filling. This model computes the dynamic response of the fluid in the tubes, the heat transfer between the fluid and the borehole filling, and the heat storage within the fluid and the borehole filling. </p>
...
Change extended models to the ones with input parameters. Added real expressions to calculate absolute convective resistances for dummy segment length. Calculate actual resistance by multiplying this dummy value by the dummy length and devide by actual length. 
</li>
<li>
