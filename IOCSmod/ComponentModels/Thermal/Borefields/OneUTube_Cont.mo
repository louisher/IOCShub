within IOCSmod.ComponentModels.Thermal.Borefields;
model OneUTube_Cont "Single U-tube borefield model with a continuous ground model"
  extends
    IOCSmod.ComponentModels.Thermal.Borefields.BaseClasses.PartialBorefield_ContRecord(
      redeclare
      IOCSmod.ComponentModels.Thermal.Borefields.BaseClasses.Boreholes.OneUTube
      borHol)
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)));
  annotation (Documentation(revisions="<html>
<ul>
<li>
July 15, 2024, by Louis Hermans:<br/>
First implementation based on work of I. Cupeiro.
</li>
<ul>
</html>"));
end OneUTube_Cont;
