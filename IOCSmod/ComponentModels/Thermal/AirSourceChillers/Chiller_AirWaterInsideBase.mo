...
model Chiller_AirWaterInsideBase
extends IDEAS.Fluid.Interfaces.PartialFourPortInterface;

  parameter Modelica.Units.SI.PressureDifference dp_nominalEva=1000
    "Pressure difference at evaporator"
    annotation (Dialog(group="Nominal condition"));
  parameter Modelica.Units.SI.PressureDifference dp_nominalCon=1000
  ...
  Modelica.Blocks.Sources.RealExpression Q_eva(final y=if on_internal then -QEva else 0)
    "Evaporator thermal power"
    annotation (Placement(transformation(extent={{84,62},{24,82}})));
  Modelica.Blocks.Sources.RealExpression EERExp(
    final y=max(1, EERDef  + coeffCon*(TConIn.T - TAir_nominal) + coeffEva*(TEvaOut.T - TEvaOut_nominal)))
    "Chiller EER"
    annotation (Placement(transformation(extent={{84,74},{24,94}})));
  Modelica.Blocks.Sources.RealExpression P_chiller(final y=if on_internal then
        QEva/EER else 0)
  ...
  IDEAS.Fluid.Sensors.TemperatureTwoPort TConOut(
    ...
    annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-50,-60})));
  IDEAS.Fluid.Sensors.TemperatureTwoPort TConIn(
    tau=0,
    m_flow_nominal=con.m_flow_nominal,
    redeclare package Medium = Medium2,
    allowFlowReversal=allowFlowReversal2) "Outlet temperature at condenser"
  ...
  IDEAS.Fluid.Sensors.TemperatureTwoPort TEvaIn(
    ...
    allowFlowReversal=allowFlowReversal1) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-70,60})));
  Modelica.Blocks.Sources.RealExpression Q_con(final y=if on_internal then QEva*(EERDef + 1)/EERDef
         else 0)
  ...
  Modelica.Blocks.Interfaces.BooleanInput on
    ...
    "On/off signal" annotation (
      Placement(transformation(
        ...
        rotation=270,
        origin={-80,110})));
  Modelica.Blocks.Interfaces.RealOutput QEva=mod*smoothMin*4180*dT_max annotation (
  ...
  Modelica.Blocks.Interfaces.RealOutput TEva "Evaporator outlet temperature"
                                                                            annotation (
...
equation
  ...
  connect(con.port_a, TConIn.port_b)
    annotation (Line(points={{10,-60},{40,-60}}, color={0,127,255}));
  connect(TConIn.port_a, port_a2)
    annotation (Line(points={{60,-60},{100,-60}}, color={0,127,255}));
  ...
  connect(TEvaOut.T, TEva) annotation (Line(points={{30,71},{16,71},{16,78},{14,
          78},{14,90},{20,90},{20,110}}, color={0,0,127}));
  connect(EERExp.y, EER) annotation (Line(points={{21,84},{-20,84},{-20,86},{-60,
          86},{-60,110}}, color={0,0,127}));
  annotation (
...
end Chiller_AirWaterInsideBase;
