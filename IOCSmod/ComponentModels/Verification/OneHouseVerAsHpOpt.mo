...
model OneHouseVerAsHpOpt
  ...
  Thermal.SizeOpt.AsHpOpt
               AsHp(
    addDummyEquation=addDummyEquation, inv_cost=900)
...
equation
  connect(AsHp.port_a, vol.ports[1]) annotation (Line(points={{-38,-2},{-38,-6},
          {-7.2,-6},{-7.2,0}}, color={0,127,255}));
  connect(AsHp.port_b, vol.ports[2]) annotation (Line(points={{-50,-2},{-50,-12},
          {-5.6,-12},{-5.6,0}}, color={0,127,255}));
  connect(buildings.port_a, vol.ports[3]) annotation (Line(points={{46,0},{46,-20},
          {-4,-20},{-4,0}}, color={0,127,255}));
  connect(buildings.port_b, vol.ports[4]) annotation (Line(points={{34,0},{34,-16},
          {-6,-16},{-6,0},{-2.4,0}}, color={0,127,255}));
  connect(bou1.ports[1], vol.ports[5]) annotation (Line(points={{-84,6},{-82,6},
          {-82,-30},{-0.8,-30},{-0.8,0}}, color={0,127,255}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
        coordinateSystem(preserveAspectRatio=false)));
end OneHouseVerAsHpOpt;
