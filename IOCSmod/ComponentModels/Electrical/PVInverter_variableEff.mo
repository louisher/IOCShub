...
model PVInverter_variableEff
  ...
  Modelica.Units.SI.Efficiency eta "Variable inverter efficiency";

protected
  parameter Modelica.Units.SI.Efficiency _eta_ref = 0.9637 "Reference efficiency";

equation
  if P_PV < 60 then
    eta = 0;
  else
    eta = eta_nom/_eta_ref*(-0.0162*P_PV/P_ac0*eta_nom - 0.0059/P_PV*P_ac0/eta_nom + 0.9858);
  end if;
  P_ac = eta*P_PV;
  ...
  annotation (
    ...
    Documentation(
      info="<html><p>Model for an inverter which can be used to transform the DC power of a PV module into an AC power. 
              ...
              <p>This model is a simple inverter model and uses a variable efficiency curve 'eta'
              and automatically curtails power when the inverter rated AC power is exceeded. The CEC (California
              Energy Commission) efficiency curve, presented in the PVWatts V5 manual is used.</p></html>",
      revisions="<html>
<ul>
